...
export function bevelMesh(input,width=.05,{segments=1,profile=.5,edges=null,weights={},dissolve=true,selection='all',angle=30}={}){
    validateMesh(input);
    ...
    if(!['all','sharp'].includes(selection)||!Number.isFinite(angle)||angle<0||angle>180)throw Error('Invalid bevel edge selection');
    ...
    const offsets=new Map();for(const [key,es] of adj){if(es.length!==2)throw Error('Invalid bevel adjacency');const w=weights[key]??1;if(!Number.isFinite(w)||w<0||w>1)throw Error('Bevel weights must be in [0, 1]');offsets.set(key,(!selected||selected.has(key))&&dot(ns[es[0].fi],ns[es[1].fi])<(selection==='sharp'?Math.min(1-1e-10,Math.cos(angle*Math.PI/180)):1-1e-10)?width*w:0);}

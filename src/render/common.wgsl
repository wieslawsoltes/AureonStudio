...
fn evaluateBSDF(m:Material,color:vec3f,n:vec3f,wo:vec3f,wi:vec3f)->BSDF {
 if(m.pattern.x<0.&&fiberRadius>0.){return evaluateFiber(m,n,wo,wi);}let ni=max(dot(n,wi),0.);let no=max(dot(n,wo),0.);if(ni<=0.||no<=0.){return BSDF(vec3f(0.),0.);}let h=normalize(wi+wo);let nh=max(dot(n,h),0.);let oh=max(dot(wo,h),1e-6);let alpha=max(.025,m.base.w*m.base.w);let f0=mix(vec3f(pow((m.params.z-1.)/(m.params.z+1.),2.)),color,m.params.x);let F=fresnelSchlick(oh,f0);let D=ggxD(nh,alpha);let G=smithG1(ni,alpha)*smithG1(no,alpha);let spec=D*G*F/max(1e-6,4.*ni*no);let diffuse=(vec3f(1.)-F)*(1.-m.params.x)*color/PI;let ps=.25+.5*m.params.x;let pdf=mix(ni/PI,D*nh/(4.*oh),ps);return BSDF((diffuse+spec)*(1.-m.params.y),pdf*(1.-m.params.y));}
...
fn fiberLobes(m:Material,wo:vec3f)->FiberLobes {
 let desc=graphCode[u32(cam.assetInfo.x)+u32(m.pattern.z)];

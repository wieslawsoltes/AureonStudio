...
struct Camera {eye:vec4f, right:vec4f, up:vec4f, forward:vec4f, size:vec4f, lens:vec4f, info:vec4f, vp:mat4x4f, misc:vec4f,volumeMin:vec4f,volumeMax:vec4f,volumeColor:vec4f,render:vec4f,tile:vec4f,sampling:vec4f};
...
fn surfaceMaterial(index:u32,p:vec3f,uv:vec2f)->Material {
 ...
 for(var i=0u;i<count;i++) {let code=graphCode[start+i];let op=u32(code.op.x);let ai=u32(max(code.args.x,0.));let bi=u32(max(code.args.y,0.));let ci=u32(max(code.args.z,0.));let a=values[ai];let b=values[bi];let c=values[ci];var v=code.value;
  switch(op) {
   ...
   case 11u: {v=textureSampleLevel(bitmapArray,bitmapSampler,a.xy,i32(code.extra.y),cam.sampling.w);}

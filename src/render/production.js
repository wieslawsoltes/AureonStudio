...
export async function renderProduction(renderer,source,{width=640,height=480,samples=source.settings.samples,shutter=source.settings.shutter||[0,0],frame=source.animation.frame,signal,onProgress=()=>{}}={}){
    ...
    const geometry=new GeometryCache();renderer.fixedSize={width,height};renderer.mode='trace';renderer.paused=false;renderer.tile=null;renderer.sampleStart=0;await renderer.setAssets(source);renderer.reset();let bvh=null,previousTime=null,doc=null;
